...
// I'm not sure why the below function wasn't working, but I'm guessing it has something to do with asking
// isNaN() to return false, because everything was returning false.
...
function canWeDeliver(zipCode) {
    if (isNaN(zipCode)) {
        console.log(`Sorry, "${zipCode}" is not a valid zip code`)
    } else {
        if (deliveryAreaZipCodes.includes(zipCode)) {
            console.log(`${zipCode} is in our delivery zone!`)
        }
        else {
            console.log(`Sorry, ${zipCode} is not within our delivery zone.`)
        }
    }
}
...
canWeDeliverTwo("Hello")
canWeDeliverTwo(85205)
canWeDeliverTwo(84057) //I'm not sure how to get the function to run an individual "is not included script"
